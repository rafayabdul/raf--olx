import React from "react";
import { useEffect } from "react";
import Header from "../../components/Header/Header";
import Footer from "../../components/Footer/Footer";
import CategorySidebar from "../../components/CategorySidebar/CategorySidebar";

const CategoryMusic = () => {
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);
    return (
        <>
        <div className="wrapper">
            <Header />
            
            <div className="homeMain_body">
                <div className="container">
                    <div className="categories_page">
                        <div className="row">
                            <div className="col-md-3">
                                <CategorySidebar />
                            </div>
                            <div className="col-md-9">
                                <div className="catmain_content">
                                    <div className="heading">
                                        <h2>Music in Dubai, UAE
                                            · 1546 ads</h2>
                                       
                                    </div>
                                    <div className="brand_box">
                                        <div className="breadcrumbs">
                                            <p>Browse results in: <a href="#">Dubai</a>  <a href="#">Music</a> Music</p>
                                        </div>
                                        <div className="brandsAmount">
                                            <div className="row">
                                                <div className="col-md-3 col-sm-4">
                                                    <div className="brandtag">
                                                        <a href="citiyana-motors-detail.html">
                                                            <p>Audi <span>(826)</span></p>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="showAll brand">
                                                <p>Show All Brands <i className="fas fa-chevron-down"></i></p>
                                            </div>
                                            <div className="brand hidden_farmat">
                                                <div className="row">
                                                    <div className="col-md-3 col-sm-4">
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Audi <span>(826)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>BMW <span>(1062)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Chevrolet <span>(734)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div className="col-md-3 col-sm-4">
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Hyundai <span>(712) </span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Mercedez Benz <span>(2897)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Mitsubishi <span>(826)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-3 col-sm-4">
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Ford <span>(734)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>and Rover <span>(1062)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Hyundai <span>(712)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-3 col-sm-4">
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>HPorshe <span>(2897)</span> </p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Toyota <span>(712)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>    
                                                    <div className="col-md-3 col-sm-4">    
                                                        <div className="brandtag">
                                                            <a href="citiyana-motors-detail.html">
                                                                <p>Nissan <span>(1062)</span></p>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="categoryHeading">
                                        <div className="left">
                                            <p>57,713 Ads in Dubai</p>
                                        </div>
                                        <div className="right">
                                            <p>SORT:
                                                <select id="filtercar">
                                                    <option value="" hidden="">Newest First</option>
                                                    <option value="DESC">DESC</option>
                                                    <option value="ASC">ASC</option>
                                                </select>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="cateList Box">
                                        <div className="image">
                                            <a href="citiyana-motors-detail.html">
                                                <img src="assets/images/motors/car01.jpg" alt="" className="" />
                                            </a>
                                        </div>
                                        <div className="catDetail">
                                            <div className="heartFavour">
                                                <a href="#" id="favourite"><i className="far fa-heart"></i></a>
                                            </div>
                                            <div className="heading">
                                                <a href="citiyana-motors-detail.html">
                                                <h2>Mercedes benz S 550 Model 2017 import from us... </h2>
                                                </a>
                                            </div>
                                            <div className="breadcrum">
                                                <ul>
                                                    <li><a href="#">Music</a></li>
                                                    <li><a href="#">Mercedes-Benz</a></li>
                                                    <li><a href="#">Mercedes-Benz S-className</a></li>
                                                </ul>
                                            </div>
                                            <div className="date">
                                                <p>25 June 2020</p>
                                            </div>
                                            <div className="brabdPrice">
                                                <h2>ETB 115,000</h2>
                                                <span>Selling Price</span>
                                            </div>
                                            <div className="brandProperty">
                                                <div className="row">
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Selling Price</p>
                                                            <span>2016</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Kilometers:</p>
                                                            <span>56,000</span>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Doors:</p>
                                                            <span>5+ doors</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Color:</p>
                                                            <span>blue</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="brandAddress">
                                            <p><i className="fas fa-map-marker-alt"></i> &nbsp; Located:  
                                                <a href="#">UAE</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Dubai</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Business Bay</a>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="cateList Box">
                                        <div className="image">
                                            <a href="#">
                                            <img src="assets/images/motors/car02.jpg" alt="" className="" />
                                            </a>
                                        </div>
                                        <div className="catDetail">
                                            <div className="heartFavour">
                                                <a href="#" id="favourite"><i className="far fa-heart"></i></a>
                                            </div>
                                            <div className="heading">
                                                <a href="citiyana-motors-detail.html">
                                                    <h2>Mercedes benz S 550 Model 2017 import from us... </h2>
                                                </a>
                                            </div>
                                            <div className="breadcrum">
                                                <ul>
                                                    <li><a href="#">Music</a></li>
                                                    <li><a href="#">Mercedes-Benz</a></li>
                                                    <li><a href="#">Mercedes-Benz S-className</a></li>
                                                </ul>
                                            </div>
                                            <div className="date">
                                                <p>25 June 2020</p>
                                            </div>
                                            <div className="brabdPrice">
                                                <h2>ETB 115,000</h2>
                                                <span>Selling Price</span>
                                            </div>
                                            <div className="brandProperty">
                                                <div className="row">
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Selling Price</p>
                                                            <span>2016</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Kilometers:</p>
                                                            <span>56,000</span>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Doors:</p>
                                                            <span>5+ doors</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Color:</p>
                                                            <span>blue</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="brandAddress">
                                            <p><i className="fas fa-map-marker-alt"></i> &nbsp; Located:  
                                                <a href="#">UAE</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Dubai</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Business Bay</a>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="cateList Box">
                                        <div className="image">
                                            <a href="citiyana-motors-detail.html">
                                            <img src="assets/images/motors/car03.jpg" alt="" className="" />
                                            </a>
                                        </div>
                                        <div className="catDetail">
                                            <div className="heartFavour">
                                                <a href="#" id="favourite"><i className="far fa-heart"></i></a>
                                            </div>
                                            <div className="heading">
                                                <a href="#">
                                                    <h2>Mercedes benz S 550 Model 2017 import from us... </h2>
                                                </a>
                                            </div>
                                            <div className="breadcrum">
                                                <ul>
                                                    <li><a href="#">Music</a></li>
                                                    <li><a href="#">Mercedes-Benz</a></li>
                                                    <li><a href="#">Mercedes-Benz S-className</a></li>
                                                </ul>
                                            </div>
                                            <div className="date">
                                                <p>25 June 2020</p>
                                            </div>
                                            <div className="brabdPrice">
                                                <h2>ETB 115,000</h2>
                                                <span>Selling Price</span>
                                            </div>
                                            <div className="brandProperty">
                                                <div className="row">
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Selling Price</p>
                                                            <span>2016</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Kilometers:</p>
                                                            <span>56,000</span>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Doors:</p>
                                                            <span>5+ doors</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Color:</p>
                                                            <span>blue</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="brandAddress">
                                            <p><i className="fas fa-map-marker-alt"></i> &nbsp; Located:  
                                                <a href="#">UAE</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Dubai</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Business Bay</a>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="cateList Box">
                                        <div className="image">
                                            <a href="citiyana-motors-detail.html">
                                                <img src="assets/images/motors/car04.jpg" alt="" className="" />
                                            </a>
                                        </div>
                                        <div className="catDetail">
                                            <div className="heartFavour">
                                                <a href="#" id="favourite"><i className="far fa-heart"></i></a>
                                            </div>
                                            <div className="heading">
                                                <a href="#"> 
                                                    <h2>Mercedes benz S 550 Model 2017 import from us... </h2>
                                                </a>
                                            </div>
                                            <div className="breadcrum">
                                                <ul>
                                                    <li><a href="#">Music</a></li>
                                                    <li><a href="#">Mercedes-Benz</a></li>
                                                    <li><a href="#">Mercedes-Benz S-className</a></li>
                                                </ul>
                                            </div>
                                            <div className="date">
                                                <p>25 June 2020</p>
                                            </div>
                                            <div className="brabdPrice">
                                                <h2>ETB 115,000</h2>
                                                <span>Selling Price</span>
                                            </div>
                                            <div className="brandProperty">
                                                <div className="row">
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Selling Price</p>
                                                            <span>2016</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Kilometers:</p>
                                                            <span>56,000</span>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Doors:</p>
                                                            <span>5+ doors</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Color:</p>
                                                            <span>blue</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="brandAddress">
                                            <p><i className="fas fa-map-marker-alt"></i> &nbsp; Located:  
                                                <a href="#">UAE</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Dubai</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Business Bay</a>
                                            </p>
                                        </div>
                                    </div>
                                    <div className="cateList Box">
                                        <div className="image">
                                            <a href="citiyana-motors-detail.html">
                                                <img src="assets/images/motors/car05.jpg" alt="" className="" />
                                            </a>
                                        </div>
                                        <div className="catDetail">
                                            <div className="heartFavour">
                                                <a href="#" id="favourite"><i className="far fa-heart"></i></a>
                                            </div>
                                            <div className="heading">
                                                <a href="#">
                                                    <h2>Mercedes benz S 550 Model 2017 import from us... </h2>
                                                </a>
                                            </div>
                                            <div className="breadcrum">
                                                <ul>
                                                    <li><a href="#">Music</a></li>
                                                    <li><a href="#">Mercedes-Benz</a></li>
                                                    <li><a href="#">Mercedes-Benz S-className</a></li>
                                                </ul>
                                            </div>
                                            <div className="date">
                                                <p>25 June 2020</p>
                                            </div>
                                            <div className="brabdPrice">
                                                <h2>ETB 115,000</h2>
                                                <span>Selling Price</span>
                                            </div>
                                            <div className="brandProperty">
                                                <div className="row">
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Selling Price</p>
                                                            <span>2016</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Kilometers:</p>
                                                            <span>56,000</span>
                                                        </div>
                                                    </div>
                                                    <div className="col-md-4">
                                                        <div className="proprty_box">
                                                            <p>Doors:</p>
                                                            <span>5+ doors</span>
                                                        </div>
                                                        <div className="proprty_box">
                                                            <p>Color:</p>
                                                            <span>blue</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="brandAddress">
                                            <p><i className="fas fa-map-marker-alt"></i> &nbsp; Located:  
                                                <a href="#">UAE</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Dubai</a> <i className="fas fa-chevron-right"></i> 
                                                <a href="#">Business Bay</a>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div> 
                </div>
            </div>

            <Footer />
        </div>
        </>
    );
};
export default CategoryMusic;
